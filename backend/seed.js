...
// ─── DATA CONFIGURATION ──────────────────────────────────────────
// These objects define the static data used to generate jobs and companies.
...
// ─── HELPER FUNCTIONS ──────────────────────────────────────────
// These functions help in generating dynamic content like logos and dates.
...
// ─── MAIN SEEDING LOGIC ────────────────────────────────────────
// This is the core function that cleans the DB and generates new records.
...
const seed = async () => {
  try {
    ...
    const categoryCounts = {
      ...
      "Startups/Ops": 10,
      "Design & Creative": 10
    }; // Total exactly 100 for the general pool
    ...
    // Specific top companies requested by user
    const userTopCompanies = [
      "Amazon", "Google", "Microsoft", "Infosys", "Wipro",
      "Tesla", "Swiggy", "Zomato", "SAP", "Spotify", "PayPal", "Uber"
    ];

    // Create a balanced pool of 20 companies for Page 1 & 2
    const page1And2Pool = [...userTopCompanies];
    // Fill remaining 8 slots to make it 20 (each company appears 1-2 times)
    const bonus = ["Amazon", "Google", "Microsoft", "Infosys", "Wipro", "Tesla", "Swiggy", "Zomato"];
    page1And2Pool.push(...bonus);

    // Shuffle to avoid having all Amazon jobs at the top
    const shuffledTopPool = page1And2Pool.sort(() => Math.random() - 0.5);

    const allCompaniesList = Object.keys(companyData).filter(c => !userTopCompanies.includes(c));
    let companyPool = [];
    ...
    for (const category of categories) {
      ...
      for (let i = 0; i < targetCount; i++) {
        ...
        allJobs.push({
          ...
          createdAt: getRandomDate(15) // Distributed over last 15 days
        });
      }
    }

    // Insert the 20 Premium Jobs LAST so they appear on Page 1 and 2 (sorted by -createdAt)
    for (const companyName of shuffledTopPool) {
      const companyObj = companyDocs[companyName];
      if (companyObj) {
        ...
        allJobs.push({
          ...
          company: companyObj._id,
          postedBy: employer._id,
          createdAt: getRandomDate(3) // Premium jobs are more recent (last 3 days)
        });
      }
    }
